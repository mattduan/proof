...
This module uses the datetime package for handling date/time types.
"""
...
try:
    import datetime # new packaging
    from datetime import date as DateType
    from datetime import datetime as DateTimeType
    from datetime import timedelta as DateTimeDeltaType

    def DateFromTicks(ticks):
	return apply(datetime.date, localtime(ticks)[:3])

    def TimeFromTicks(ticks):
	return apply(datetime.time, localtime(ticks)[3:6])

    def TimestampFromTicks(ticks):
	return apply(datetime.datetime, localtime(ticks)[:6])
    ...
    def mysql_timestamp_converter(s):
        try:
            ...
            return apply(datetime.datetime, tuple(parts))
        except ValueError:
            return None

    def mysql_datetime_converter(s):
        try:
            parts = tuple(time.strptime(s, '%Y-%m-%d %H:%M:%S'))
            return apply(datetime.datetime, parts[:6])
        except ValueError:
            return None

    def mysql_date_converter(s):
        try:
            parts = tuple(time.strptime(s, '%Y-%m-%d'))
            return apply(datetime.date, parts[:3])
        except ValueError:
            return None

    def mysql_time_converter(s):
        try:
            sign = s[0]=="-" and s[0] or ""
            s    = s[0]=="-" and s[1:] or s
            parts = tuple(time.strptime(s, '%H:%M:%S'))
            if sign:
    ...
    type_conv[FIELD_TYPE.DATETIME]  = mysql_datetime_converter
    type_conv[FIELD_TYPE.TIME]      = mysql_time_converter
    type_conv[FIELD_TYPE.DATE]      = mysql_date_converter
...
class BaseCursor:
    ...
    def __init__(self, connection, result_type=0):
    ...
    def _fetch_row(self, fetch_type=None):
        _fetch_type = fetch_type or self._fetch_type
        r = self._result.fetch_row(1, _fetch_type)
        return r and r[0] or ()

    def _fetch_rows(self, size, fetch_type=None):
        _fetch_type = fetch_type or self._fetch_type
        return self._result.fetch_row(size, _fetch_type)

    def _fetch_all_rows(self, fetch_type=None): 
        _fetch_type = fetch_type or self._fetch_type
        return self._result.fetch_row(0, _fetch_type)
